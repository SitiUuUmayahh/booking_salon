...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="px-4 py-3">
                <div class="space-y-2.5">
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="text-sm font-semibold text-gray-900">{{ $booking->created_at->format('d M Y, H:i') }} WIB</span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm font-medium text-gray-600">Terakhir Update:</span>
                        <span class="text-sm font-semibold text-gray-900">{{ $booking->updated_at->format('d M Y, H:i') }} WIB</span>
                    </div>
                    @if($booking->dp_paid_at)
                        <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <span class="text-sm font-medium text-blue-700">DP Diupload:</span>
                            <span class="text-sm font-semibold text-blue-900">{{ $booking->dp_paid_at->format('d M Y, H:i') }} WIB</span>
                        </div>
                    @endif
                    @if($booking->dp_verified_at)
                        <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
                            <span class="text-sm font-medium text-green-700">DP Diverifikasi:</span>
                            <span class="text-sm font-semibold text-green-900">{{ $booking->dp_verified_at->format('d M Y, H:i') }} WIB</span>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="space-y-4">
        ...
        @if($booking->dp_status === 'pending')
            <div class="bg-white rounded-xl shadow-lg overflow-hidden border-2 border-yellow-300">
                ...
                <div class="p-4 space-y-3">
                    <form method="POST" action="{{ route('admin.bookings.verify-dp', $booking->id) }}">
                        @csrf
                        <button type="submit" class="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold py-3.5 px-4 rounded-lg transition transform hover:scale-102 shadow-lg">
                    ...
                    <button onclick="document.getElementById('rejectModal').classList.remove('hidden')"
                            class="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-bold py-3.5 px-4 rounded-lg transition transform hover:scale-102 shadow-lg">
                        <span class="flex items-center justify-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- WhatsApp Contact -->
        @if($booking->dp_status === 'pending' || $booking->dp_status === 'verified')
            ...
            <div class="p-4 space-y-3">
                @if($booking->status === 'pending')
                    @if($booking->dp_status === 'verified')
                        <form method="POST" action="{{ route('admin.bookings.confirm', $booking->id) }}">
                            @csrf
                            <button type="submit" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold py-3.5 px-4 rounded-lg transition transform hover:scale-102 shadow-lg">
                                <span class="flex items-center justify-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        <div class="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-5">
                            <div class="flex flex-col items-center justify-center text-center">
                                <svg class="w-12 h-12 text-gray-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                </svg>
                                <p class="text-gray-700 font-semibold text-sm mb-1">Konfirmasi Booking Terkunci</p>
                                <p class="text-gray-500 text-xs">Verifikasi DP terlebih dahulu</p>
                            </div>
                        </div>
                    @endif
                @endif
                ...
                @if($booking->status === 'confirmed')
                    <form method="POST" action="{{ route('admin.bookings.complete', $booking->id) }}">
                        @csrf
                        <button type="submit" class="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold py-3.5 px-4 rounded-lg transition transform hover:scale-102 shadow-lg">
                            <span class="flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
